...
if ((isset($_GET['sil'])) && ($_GET['sil'] != "")) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM resim WHERE uyeID = :uyeID AND ID = :silID AND yer = 'sertifika'");
    $stmt->execute(['uyeID' => $row_uyebilgileri['uyeID'], 'silID' => $silID]);
    $row_sertifikaresim = $stmt->fetch(PDO::FETCH_ASSOC);
  
    if ($row_sertifikaresim) {
        unlink("../../resim/sertifika/" . $row_sertifikaresim['resim']);
        $stmt = $pdo->prepare("DELETE FROM resim WHERE ID = :silID");
        $stmt->execute(['silID' => $silID]);

        header("Location: index1.php?Sil=SilmeBasarili");
        exit;
    }
}
