...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["MM_insert"]) && $_POST["MM_insert"] === "form1") {
  $uyeID = filter_input(INPUT_POST, 'uyeID', FILTER_SANITIZE_NUMBER_INT);
  $yer = filter_input(INPUT_POST, 'yer', FILTER_SANITIZE_STRING);

  foreach ($_FILES['resim']['name'] as $f => $name) {
    if ($_FILES['resim']['error'][$f] == 0 && !empty($name)) {
      $ext = pathinfo($name, PATHINFO_EXTENSION);
      // Sadece belirli dosya türlerine izin ver
      if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif'])) {
        $newName = md5(uniqid(rand(), true)) . '.' . $ext;
        $destination = "../../resim/sirket/" . $newName;
        if (move_uploaded_file($_FILES['resim']['tmp_name'][$f], $destination)) {
          $stmt = $pdo->prepare("INSERT INTO resim (uyeID, yer, resim) VALUES (?, ?, ?)");
          $stmt->execute([$uyeID, $yer, $newName]);
        }
      }
    }
  }

  header("Location: index.php?Ekle=EklemeBasarili" . (isset($_SERVER['QUERY_STRING']) ? '&' . $_SERVER['QUERY_STRING'] : ''));
  exit;
}
...
<div class="span12">
    <?php if (empty($row_sirketresim)) { ?>
        <img src="../../resim/yok.png" width="50" height="50">
    <?php } else { ?>
        <?php foreach ($row_sirketresim as $resim) { ?>
            <div class="span1">
                <div class="thumbnail">
                    <div class="item">
                        <a class="fancybox-button" data-rel="fancybox-button" title="Photo" href="../../resim/sirket/<?php echo htmlspecialchars($resim['resim']); ?>">
                            <div class="zoom">
                                <img src="../../resim/sirket/<?php echo htmlspecialchars($resim['resim']); ?>" width="100" height="100" alt="Photo">
                                <div class="zoom-icon"></div>
                            </div>
                        </a>
                    </div>
                </div>
                <a href="sil.php?sil=<?php echo htmlspecialchars($resim['ID']); ?>"><i class="icon-remove"></i><?php echo $dil['sil']; ?></a>
            </div>
        <?php } ?>
    <?php } ?>
</div>
...
<div class="widget">
      ...
      <div class="row-fluid">
          <div class="span6">       
                  ...
                  <input name="yer" type="hidden" class="span4 " id="yer" value="sirket">
