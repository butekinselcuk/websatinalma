...
if (isset($_POST["MM_insert"]) && $_POST["MM_insert"] == "form1") {
    $query = "INSERT INTO sohbet (sohbetTalepID, kime, mesaj, kimden, durum, ID) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $pdo->prepare($query);
    $stmt->execute([
        $_POST['sohbetTalepID'], $_POST['kime'], $_POST['mesaj'], $_POST['kimden'], $_POST['durum'], $_POST['ID']
    ]);
...
$kime = filter_input(INPUT_GET, 'kime', FILTER_SANITIZE_NUMBER_INT);
...
<?php
// Kullanıcıdan alınan 's' parametresini güvenli bir şekilde al
$sil = filter_input(INPUT_GET, 's', FILTER_SANITIZE_STRING);

if ($sil) {
    // Hazırlanan sorguyu PDO ile güvenli bir şekilde çalıştır
    $stmt = $pdo->prepare("UPDATE sohbet SET sil = '1', durum = '1' WHERE sohbetID = :sil");
    $stmt->execute(['sil' => $sil]);
}
?>
